// ActionMoviesComponent.js
import React, { useState, useEffect } from 'react';
import { View, Text, StyleSheet, ScrollView, Image, ActivityIndicator, TouchableOpacity } from 'react-native';
import { Feather } from '@expo/vector-icons';
import { useNavigation } from '@react-navigation/native';

const ActionMoviesComponent = () => {
    ...
    const fetchActionMovies = async () => {
      try {
        ...
        const response = await fetch(
          `https://api.themoviedb.org/3/discover/movie?api_key=${process.env.API_KEY}&with_genres=28&primary_release_date.gte=${formattedFiveMonthsAgo}&primary_release_date.lte=${formattedCurrentDate}`
        );
        ...
        setError('An error occurred while fetching action movies data.');
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerText}>Recent Action</Text>
      </View>
...
export default ActionMoviesComponent;
